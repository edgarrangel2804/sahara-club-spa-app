...
class AdminMessagesPage extends StatefulWidget {
  ...
  const AdminMessagesPage({super.key, required this.repo});
...
class _AdminMessagesPageState extends State<AdminMessagesPage> {
  ...
  Widget _buildBanner() {
    ...
    final receptionists = _contacts.where((c) => c['role'] == 'receptionist').length;
  ...
  Widget _buildContactCard(Map<String, dynamic> c) {
    ...
    final roleColor   = isTherapist ? SaharaColors.gold : const Color(0xFF60A5FA);
    ...
    final roleLabel = isTherapist
        ? (specialty != null ? _fmtSpecialty(specialty) : 'Terapeuta')
        : 'Recepcionista';
            ...
            Container(
              ...
              child: Center(
                child: Text(initials, style: GoogleFonts.inter(
                  fontSize: 14, color: roleColor, fontWeight: FontWeight.w700,
                )),
              ),
            ),
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Text(roleLabel, style: GoogleFonts.inter(
                      fontSize: 10, color: roleColor, fontWeight: FontWeight.w500,
                    )),
                  ),
                ],
              ),
...
class _AdminChatPageState extends State<AdminChatPage> {
  ...
  String get _myId => widget.repo.myId;
  ...
  Future<void> _initChat() async {
    ...
    setState(() {
      _chatId       = chatId;
      _stream       = widget.repo.streamMessages(chatId);
      _initializing = false;
    });
  }
        ...
        return ListView.builder(
          ...
          itemBuilder: (_, i) {
            ...
            final showDate = prev == null ||
                ...
                _dayOf(prev['created_at'] as String? ?? '');
            return Column(
              children: [
                if (showDate)
                  _DateSeparator(dateStr: msg['created_at'] as String? ?? ''),
                _MessageBubble(
                  msg:  msg,
                  mine: (msg['sender_id'] as String?) == _myId,
                ),
              ],
            );
          },
...
class _MessageBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    try {
      ...
      time = DateFormat('HH:mm').format(d);
    } catch (_) {}
